...
public class CreateAccountCrud implements EventListener<Event> {
	protected MTForm mtForm;
	IMTComponent paymentTypeSelector;
	...
	public Component buildEditorScreen(Component comp,boolean isModal) throws WrongValueException, SQLException {
		try {
			...
			zulBuilder.setZulURI("~./zul/poyake-roadrunner/screen/account/createWithMtEditor.zul");
			...
			paymentTypeSelector = (IMTComponent) mtForm.getFormComponent().getFellowIfAny("PAYMENT_TYPE_SELECTOR");
	...
	void setPaymentType(Event evt) {
		...
		Integer paymentTypeId = (Integer) paymentTypeSelector.getValue();
		if (null != paymentTypeId) {
			if (paymentTypeId == EnuPaymentType.VISA
					|| paymentTypeId == EnuPaymentType.MCARD
					|| paymentTypeId == EnuPaymentType.AMEX
					|| paymentTypeId == EnuPaymentType.DISC) {
				mtForm.getFormComponent().getFellowIfAny("CREDIT_CARD_NUMBER").setVisible(true);
			}
			if (paymentTypeId == EnuPaymentType.DIRECTDEBIT) {
				mtForm.getFormComponent().getFellowIfAny("ACCOUNT_NUMBER").setVisible(true);
			}
			if (paymentTypeId == EnuPaymentType.CHECK) {
