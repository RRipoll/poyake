...
public class CustomerFieldRenderer implements IMTFieldRenderer {
	...
	public String render(Object value, MTField mtField, IDetachedRecord values, Locale locale) {
		...
		DBQuery dqr= new DBQuery(MTroadRunnerData.HPERSONORCOMPANY);
		...
		try {
			...
			if (null!=row) {
				for (MTField description : MTroadRunnerData.HPERSONORCOMPANY.getDescriptionFields()) {
	...
	public MTField forField() {
		...
		return MTroadRunnerData.CUSTOMERPK.CUSTOMERID;
	}
