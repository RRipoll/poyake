...
import com.etantolling.testrunner.test.zkweb.datagrid3ws.filter.BasicDataFilter;
...
public class DataTable extends Div implements EventListener<Event> {
	...
	public boolean isOnlyPageCheckbox=false;
	...
	public DataTable(DataGridModel dgModel) throws WrongValueException, Exception{
	...
	public DataTable(DataGridModel dgModel, DataGridFilter filter, BasicRowAdapter rowAdapter, EventListener<Event> eventListener, String cssClass) throws Exception {

		init(dgModel,filter,rowAdapter,eventListener,DataTable.PAGING,cssClass);
		
	}
	
	public DataTable(DataGridModel dgModel, DataGridFilter filter, BasicRowAdapter rowAdapter, EventListener<Event> eventListener,Integer pagingType, String cssClass) throws Exception {
		
		init(dgModel,filter,rowAdapter,eventListener,pagingType,cssClass);
		
	}

	public DataTable(DataGridModel dgModel, DataGridFilter filter, BasicRowAdapter rowAdapter, EventListener<Event> eventListener,Integer pagingType,Component pagingComponent, String cssClass) throws Exception {
	...
	public void init(DataGridModel dgModel, DataGridFilter filter, BasicRowAdapter rowAdapter, EventListener<Event> eventListener,Integer pagingType, String cssClass) throws Exception {
	...
	public void layout() throws WrongValueException, Exception {
		...
		if (null==pagingComponent && PAGING_NONE != pagingType && pagingPosition==DataTable.PAGINGPOSITION_TOP){
	...
	public Vector<LinkedHashMap<String, Object>> renderBody(Component table) throws Exception {
	...
	public void onEvent(Event event) throws Exception {
		...
		if (null != event.getTarget().getAttribute("COLUMN_ORDERBY_NAME")) {
			...
			if (headerName.equals(dgModel.getDgQuery().getOrderByField()))
	...
	public void setFilter(DataGridFilter filter) throws WrongValueException, Exception {
		this.filter = filter;
		layout();
	}
/*
	...
	*/
	public HashMap<String, Vector<LinkedHashMap<String, String>>> renderToXlsTemplate(HashMap<String, Vector<LinkedHashMap<String, String>>> beans) throws Exception, IOException,  InvalidFormatException {
		...
		for (String key : params.keySet()) {
			formatParams.put(key,DataFormatter.formatValue(params.get(key),null));
		}
		 ...
		 Vector<LinkedHashMap<String, Object>> page = dgModel.getTotalResults();
			for (LinkedHashMap<String, Object> row : page) {
				...
				for (String columnName : dgModel.getDgQuery().getColumnNames()) {
						...
						for (Component child : div.getChildren()) {
							if(child instanceof  Label){
								rowString.put(columnName, ((Label) child).getValue().trim());
								break;
							}
							else if(child instanceof  org.zkoss.zhtml.Text){
								rowString.put(columnName,((org.zkoss.zhtml.Text) child).getValue().trim());
								break;
							} else if(child instanceof  org.zkoss.zul.Textbox){
								rowString.put(columnName,((org.zkoss.zul.Textbox) child).getValue().trim());
								break;
							}  else if(child instanceof org.zkoss.zul.Html){
								rowString.put(columnName,((org.zkoss.zul.Html) child).getContent().trim());
								break;
							} if(child instanceof org.zkoss.zhtml.Html ){
								rowString.put(columnName,DataFormatter.formatValue(row.get(columnName),null).trim());
								break;
							}
						}
