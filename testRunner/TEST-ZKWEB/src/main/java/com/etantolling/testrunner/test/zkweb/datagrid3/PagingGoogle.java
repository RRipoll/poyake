...
public class PagingGoogle extends Div implements EventListener<Event> ,Pageable{
	...
	Integer pageSize=50;
	Integer totalSize=0;
	boolean showPageBox=true;
	boolean showSizePageBox=true;
	Integer PageRowCount;
	Intbox sizePageShow;
	Intbox pageShow;
	
	public boolean changeToPaging=false;
	...
	public PagingGoogle(Integer activePage, Integer pageSize, Integer totalSize) {
		...
		this.setStyle("text-align: center;cursor:pointer;");
		this.setZclass("z-paging-icon ");
		this.setSclass("z-paging-icon ");
		this.setClass("z-paging-icon ");
	...
	public void layout() {
			...
			if(null==totalSize)pageCount=100;
			if(null!=totalSize)pageCount=(int) Math.ceil(new Double(totalSize)/new Double(pageSize));
			if(pageCount>1){
				if(showSizePageBox && null!=totalSize)setShowSizePageBox();
				if(showPageBox)setPageShow();
				setPrevious();
				if(null!=totalSize)setPagesNumbers();
				else setPageNumber();
				setNext();
			}
		}
	...
	private void setNext() {
		...
		if(activePage!=pageCount-1  && PageRowCount==pageSize){
	...
	/*
	private void showButtonChange() {
			
			org.zkoss.zul.Html next=new org.zkoss.zul.Html(" *   ");
			next.setClass("pagingnext");
			next.addEventListener(Events.ON_CLICK, this);
			next.setAttribute("value","showButton");
			next.setParent(this);
...
*/
	...
	private void setPageNumber() {
			...
			org.zkoss.zul.Html a=new org.zkoss.zul.Html("  "+(activePage.intValue()+1)+"  ");
			a.setClass("pagingnumber");
			a.setParent(this);
			a.setClass("pagingactive");
	...
	public void setTotalSize(Integer totalSize) {
		this.totalSize = totalSize;
		if(null!=totalSize)pageCount=totalSize/pageSize;
	}
