...
public class MTField extends AbstractField{
	...
	public boolean equals(Object obj) {
		...
		return this.getFullyQualifiedName().equals(((MTField) obj).getFullyQualifiedName());
	}
	...
	public void overrideLabel(String labelType, String lang, String text) {
		Label label = null;
		for (Label testLabel:labels) {
			Locale locale = Label.fromLangString(lang);
			if (null != locale && testLabel.getLocale().equals(locale) && testLabel.getType().equalsIgnoreCase(labelType) )
				label = testLabel;
		}
		
		if (null == label)
			labels.add(new Label(labelType, lang, text));
		else
			label.setText(text);
	}
	...
	public boolean isGuiInput() {
		return (getStereoTypes().contains("GUIINPUT"));
	}
	...
	public MTTable getMultiRefTo() {
		if(null==multiRefTo)
			if(getTable().isView() && null!=getSameAs().getMultiRefTo())
				return getSameAs().getMultiRefTo();
		return multiRefTo;
	}
