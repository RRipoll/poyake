...
public  class MultiObjectEditorFilter  extends SqlExtended implements ISqlExtended{
	...
	public String generateWhereClause(FilterConfiguration element,MapValues<Object> parameters) {
		...
		where= linkin.getForeignKey().getPrimaryKey().getName() +" in (select "+linkin.getName()+" from "+mtField.getMultiRefTo().getFullTableName()+" where "+where+" )";	
